...
try:
    from . import test
    from .paths import generated_artifacts_root, restricted_data_root
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate locally generated HiReTest cases for one or all assignment transitions."
    )
    parser.add_argument(
        "--case-root",
        type=Path,
        default=generated_artifacts_root() / "checked_cases",
        help="Root containing Hiretest/<stage> checked-case directories generated by ask_for_llm.py.",
    )
    parser.add_argument(
        "--results-root",
        type=Path,
        default=generated_artifacts_root() / "evaluation",
        help="Writable root for local evaluation workbooks.",
    )
    parser.add_argument(
        "--students-dir",
        type=Path,
